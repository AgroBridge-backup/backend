...
export function createQualityMetricsRouter(prisma: PrismaClient): Router {
  ...
  router.get('/', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await service.listMetrics(exportCompanyId, {
        ...
        exportEligible: exportEligible === 'true' ? true : exportEligible === 'false' ? false : undefined,
   ...
   */
  router.get('/thresholds/:cropType', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const thresholds = service.getThresholds(req.params.cropType as CropType);
