...
const RegisterSensorSchema = z.object({
  ...
  deviceType: z.enum(['TEMP_HUMIDITY', 'BRIX_METER', 'MULTIFUNCTION', 'PH_METER']),
...
const StartSessionSchema = z.object({
  sessionType: z.enum(['FIELD_MONITORING', 'HARVEST_COOLING', 'COLD_STORAGE', 'TRANSPORT']),
  cropType: z.enum(['AVOCADO', 'BLUEBERRY', 'STRAWBERRY', 'RASPBERRY', 'COFFEE', 'CACAO']),
...
export function createColdChainRouter(prisma: PrismaClient): Router {
   ...
   */
  router.post('/sensors', async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   * Get sensor by ID
   */
  router.get('/sensors/:id', async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   */
  router.patch('/sensors/:id/assign', async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   */
  router.patch('/sensors/:id/status', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { status } = req.body;

      if (!['ACTIVE', 'OFFLINE', 'LOW_BATTERY', 'MAINTENANCE', 'DECOMMISSIONED'].includes(status)) {
   ...
   */
  router.get('/sessions/:id/readings', async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   */
  router.get('/sessions/:id/report', async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  router.post('/readings', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const reading = await service.recordReading({
        ...data,
        readingTime: data.readingTime ? new Date(data.readingTime) : undefined,
      });
   ...
   */
  router.post('/readings/bulk', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        readings.map(async (reading: any) => {
          try {
            ...
            return await service.recordReading({
              ...validated,
              readingTime: validated.readingTime ? new Date(validated.readingTime) : undefined,
            });
          } catch (error) {
            return { error: error instanceof Error ? error.message : 'Unknown error', input: reading };
          }
        })
      );
