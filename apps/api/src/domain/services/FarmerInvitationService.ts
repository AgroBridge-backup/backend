...
export class FarmerInvitationService {
  ...
  async sendInvitation(
    input: CreateFarmerInvitationInput,
    baseUrl: string,
    ...
    // Check capacity
    const hasCapacity = await this.companyRepository.hasCapacityForFarmers(
      input.exportCompanyId,
    );
    if (!hasCapacity) {
      throw new AppError(
        "Company has reached farmer limit for current tier",
        403,
      );
    }
    ...
    logger.info(
      `Farmer invitation sent: ${invitation.id} to ${invitation.email}`,
    );
  ...
  async validateToken(token: string): Promise<ValidateTokenResult> {
    ...
    if (
      invitation.status === FarmerInvitationStatus.EXPIRED ||
  ...
  async acceptInvitation(
    ...
    logger.info(
      `Farmer invitation accepted: ${invitation.id} by farmer ${farmerId}`,
    );
  ...
  async resendInvitation(
    ...
    const updatedInvitation = await this.invitationRepository.resend(
      id,
      newToken,
      newExpiresAt,
    );
  ...
  async getInvitationStats(companyId: string): Promise<{
    ...
    const [pending, accepted, expired, cancelled] = await Promise.all([
      this.invitationRepository.list({
      ...
      this.invitationRepository.list({
        exportCompanyId: companyId,
        status: FarmerInvitationStatus.ACCEPTED,
        limit: 0,
      }),
      this.invitationRepository.list({
        exportCompanyId: companyId,
        status: FarmerInvitationStatus.EXPIRED,
        limit: 0,
      }),
      this.invitationRepository.list({
        exportCompanyId: companyId,
        status: FarmerInvitationStatus.CANCELLED,
        limit: 0,
      }),
    ]);
