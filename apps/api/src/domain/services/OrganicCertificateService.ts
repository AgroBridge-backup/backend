...
export class BlockchainAnchoringError extends Error {
  ...
  constructor(public originalError: Error, public retryable: boolean) {
...
export interface OrganicCertificateServiceDeps {
  ...
  storageUploader?: (key: string, buffer: Buffer, contentType: string) => Promise<string>;
}
...
export class OrganicCertificateService {
  ...
  private readonly storageUploader?: (key: string, buffer: Buffer, contentType: string) => Promise<string>;
   ...
   */
  async generateCertificate(request: CertificateGenerationRequest): Promise<OrganicCertificate> {
    ...
    try {
      ...
      if (!farmer.exportCompanyId || !farmer.exportCompany) {
        throw new AppError('Farmer is not associated with an export company', 400);
      }
      ...
      for (const fieldId of request.fieldIds) {
        ...
        if (field.producerId !== request.farmerId) {
          throw new AppError('Field ' + fieldId + ' does not belong to farmer', 403);
        }
      }

      // STEP 3: Check for pending certificates
      const hasPending = await this.certificateRepository.hasPendingCertificateForFields(
        request.farmerId,
        request.fieldIds
      );
      if (hasPending) {
        throw new AppError('Farmer already has a pending certificate for these fields', 409);
      }

      // STEP 4: Aggregate field inspection data
      const dateRange = {
        startDate: new Date(Date.now() - CERTIFICATE_REQUIREMENTS.inspectionWindowDays * 24 * 60 * 60 * 1000),
        endDate: new Date(),
      };
      const fieldData = await this.aggregateFieldData(request.fieldIds, dateRange);

      // STEP 5: Validate inspection requirements
      const validation = await this.validateInspectionRequirements(request.fieldIds, fieldData);
      ...
      const payloadSnapshot = this.buildPayloadSnapshot(request, fieldData, certificateNumber);
      const contentHash = this.computeHash(payloadSnapshot);

      // STEP 8: Create certificate record (status: PROCESSING)
      const validFrom = new Date();
      const validTo = new Date();
      validTo.setDate(validTo.getDate() + CERTIFICATE_REQUIREMENTS.certificateValidityDays);
      ...
      const updatedCertificate = await this.certificateRepository.update(certificate.id, {
   ...
   */
  async getCertificateWithDetails(certificateId: string): Promise<OrganicCertificateWithDetails> {
    const certificate = await this.certificateRepository.findByIdWithDetails(certificateId);
    if (!certificate) {
      throw new CertificateNotFoundError(certificateId);
    }
    return certificate;
   ...
   */
  async approveCertificate(certificateId: string, reviewerId: string): Promise<OrganicCertificate> {
    ...
    const updatedCertificate = await this.certificateRepository.update(certificateId, {
   ...
   */
  async rejectCertificate(certificateId: string, reviewerId: string, reason: string): Promise<OrganicCertificate> {
    const certificate = await this.getCertificate(certificateId);
   ...
   */
  async revokeCertificate(certificateId: string, reason: string): Promise<OrganicCertificate> {
    const certificate = await this.getCertificate(certificateId);

    if (certificate.status !== OrganicCertificateStatus.APPROVED) {
      throw new AppError('Cannot revoke certificate with status: ' + certificate.status, 400);
    }
   ...
   */
  async verifyCertificateByCertNumber(certificateNumber: string, viewData?: ViewData): Promise<PublicCertificateData> {
    const certificate = await this.certificateRepository.findByCertificateNumberWithDetails(certificateNumber);
    ...
    const publicData: PublicCertificateData = {
      ...
      farmInfo: {
        ...
        location: certificate.farmer.municipality + ', ' + certificate.farmer.state,
        totalHectares,
        cropType: certificate.cropType,
      },
      certification: {
        standard: CERTIFICATION_DISPLAY_NAMES[certificate.certificationStandard] || certificate.certificationStandard,
      ...
      blockchain: certificate.blockchainTxHash ? {
        ...
        explorerUrl: this.getExplorerUrl(certificate.blockchainNetwork || 'POLYGON', certificate.blockchainTxHash),
        verified: true,
      } : null,
   ...
   */
  async verifyBlockchainProof(certificateNumber: string): Promise<BlockchainProof> {
    ...
    if (this.blockchainService) {
      try {
        const history = await this.blockchainService.getBatchHistoryFromChain(certificate.certificateNumber);
   ...
   */
  async aggregateFieldData(fieldIds: string[], dateRange: { startDate: Date; endDate: Date }): Promise<AggregatedFieldData> {
    ...
    for (const fieldId of fieldIds) {
      ...
      const inspectionResult = await this.inspectionRepository.listByField(fieldId, {
      ...
      for (const inspection of inspectionResult.inspections) {
        const details = await this.inspectionRepository.findByIdWithDetails(inspection.id);
        if (!details) continue;
        ...
        for (const photo of details.photos) {
          allPhotos.push({
            ...
            gpsLocation: photo.latitude && photo.longitude ? { lat: photo.latitude, lng: photo.longitude } : null,
    ...
    const verifiedInspections = allInspections.filter((i) => i.isVerified).length;
    const verifiedOrganicInputs = allOrganicInputs.filter((i) => i.verificationStatus === 'VERIFIED').length;
   ...
   */
  async validateInspectionRequirements(fieldIds: string[], fieldData?: AggregatedFieldData): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];

    const data = fieldData || await this.aggregateFieldData(fieldIds, {
      startDate: new Date(Date.now() - CERTIFICATE_REQUIREMENTS.inspectionWindowDays * 24 * 60 * 60 * 1000),
      endDate: new Date(),
    });

    if (data.summary.totalInspections < CERTIFICATE_REQUIREMENTS.minInspections) {
      errors.push({
        code: 'INSUFFICIENT_INSPECTIONS',
        field: 'inspections',
        message: 'Minimum ' + CERTIFICATE_REQUIREMENTS.minInspections + ' inspections required in last ' + CERTIFICATE_REQUIREMENTS.inspectionWindowDays + ' days',
    ...
    if (data.summary.totalPhotos < CERTIFICATE_REQUIREMENTS.minPhotos) {
      errors.push({
        ...
        field: 'photos',
        message: 'Minimum ' + CERTIFICATE_REQUIREMENTS.minPhotos + ' photos required across all inspections',
        required: CERTIFICATE_REQUIREMENTS.minPhotos,
        actual: data.summary.totalPhotos,
      });
    }

    if (data.summary.totalOrganicInputs < CERTIFICATE_REQUIREMENTS.minOrganicInputs) {
      errors.push({
        code: 'INSUFFICIENT_ORGANIC_INPUTS',
        field: 'organicInputs',
        message: 'Minimum ' + CERTIFICATE_REQUIREMENTS.minOrganicInputs + ' organic inputs required',
    ...
    if (data.summary.verifiedInspections < data.summary.totalInspections) {
      warnings.push({
        ...
        field: 'inspections',
        message: (data.summary.totalInspections - data.summary.verifiedInspections) + ' inspections are not yet verified',
      });
    }
    ...
    if (data.summary.verifiedOrganicInputs < data.summary.totalOrganicInputs) {
      warnings.push({
        ...
        field: 'organicInputs',
        message: (data.summary.totalOrganicInputs - data.summary.verifiedOrganicInputs) + ' organic inputs are not yet verified',
      });
    }
  ...
  async generateUniqueCertificateNumber(): Promise<string> {
    ...
    const sequence = await this.certificateRepository.getNextSequenceNumber(year);
   ...
   */
  private async generateAndStorePdf(certificate: OrganicCertificate, fieldData: AggregatedFieldData, farmer: any): Promise<void> {
    try {
      const qrContent: QrCodeContent = {
        certificateNumber: certificate.certificateNumber,
        verificationUrl: 'https://verify.agrobridge.io/' + certificate.certificateNumber,
      ...
      const pdfData: PdfTemplateData = {
        ...
        blockchain: {
          ...
          explorerUrl: certificate.blockchainTxHash
            ? this.getExplorerUrl(certificate.blockchainNetwork || 'POLYGON', certificate.blockchainTxHash)
            : '',
        },
        exportCompany: {
      ...
      if (this.storageUploader) {
        pdfUrl = await this.storageUploader('certificates/' + certificate.certificateNumber + '.pdf', pdfBuffer, 'application/pdf');
      }

      if (pdfUrl) {
        await this.certificateRepository.update(certificate.id, { pdfUrl, qrCodeUrl: qrCodeDataUrl });
      }

      logger.info('PDF generation completed', { certificateId: certificate.id, pdfSize: pdfBuffer.length, pdfUrl });
    } catch (error: any) {
      logger.error('PDF generation failed', { certificateId: certificate.id, error: error.message });
   ...
   */
  private async anchorToBlockchainWithRetry(certificateId: string, contentHash: string): Promise<string | null> {
    if (!this.blockchainService) {
      logger.warn('Blockchain service not configured, skipping anchoring', { certificateId });
      return null;
    }
    ...
    for (let attempt = 1; attempt <= this.MAX_BLOCKCHAIN_RETRIES; attempt++) {
      try {
        ...
        logger.info('Certificate anchored to blockchain', { certificateId, txHash: result.txHash, attempt });
        return result.txHash;
      } catch (error: any) {
        logger.warn('Blockchain anchoring attempt failed', { certificateId, attempt, error: error.message });

        if (attempt === this.MAX_BLOCKCHAIN_RETRIES) {
          await this.certificateRepository.update(certificateId, { status: OrganicCertificateStatus.BLOCKCHAIN_FAILED });
          throw new BlockchainAnchoringError(error, false);
        }

        await this.sleep(this.BLOCKCHAIN_RETRY_DELAY_MS * Math.pow(2, attempt - 1));
      }
    }
   ...
   */
  private async uploadToIpfsAsync(certificate: OrganicCertificate, fieldData: AggregatedFieldData, farmer: any): Promise<void> {
    ...
    try {
      const metadata: CertificateIpfsMetadata = {
        ...
        fields: fieldData.fields.map((f) => ({
          ...
          hectares: f.hectares,
          cropType: f.cropType,
          gpsCoordinates: { centerLat: f.gpsCoordinates.centerLat, centerLng: f.gpsCoordinates.centerLng },
        })),
        ...
        blockchain: {
          txHash: certificate.blockchainTxHash || '',
          network: certificate.blockchainNetwork || 'POLYGON',
          timestamp: (certificate.blockchainTimestamp || new Date()).toISOString(),
        },
        certificationStandard: certificate.certificationStandard,
        cropType: certificate.cropType,
      ...
      const ipfsHash = await this.ipfsService.uploadCertificateMetadata(certificate.certificateNumber, metadata);

      if (ipfsHash) {
        await this.certificateRepository.update(certificate.id, { ipfsHash });
        logger.info('Certificate metadata uploaded to IPFS', { certificateId: certificate.id, ipfsHash });
      }
    } catch (error: any) {
      logger.error('IPFS upload failed', { certificateId: certificate.id, error: error.message });
    }
  }
   ...
   */
  private buildPayloadSnapshot(request: CertificateGenerationRequest, fieldData: AggregatedFieldData, certificateNumber: string): string {
    const payload = {
      version: '1.0.0',
      certificateNumber,
      farmerId: request.farmerId,
  ...
  private getExplorerUrl(network: string, txHash: string): string {
    const baseUrl = BLOCKCHAIN_EXPLORERS[network.toUpperCase()] || BLOCKCHAIN_EXPLORERS.POLYGON;
