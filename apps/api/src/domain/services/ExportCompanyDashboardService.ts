...
export class FarmerLimitExceededError extends AppError {
  constructor(maxFarmers: number, currentCount: number, attemptedToAdd: number) {
    super(
      'Farmer limit exceeded. Max: ' + maxFarmers + ', Current: ' + currentCount + ', Attempted to add: ' + attemptedToAdd,
      400
    );
...
export class ExportCompanyDashboardService {
   ...
   */
  async getDashboardStats(companyId: string, dateRange?: DateRange): Promise<DashboardStats> {
  ...
  private async getCertificateStats(companyId: string, range: DateRange) {
    ...
    const reviewedCerts = certificates.filter(
      c => c.reviewedAt && (c.status === 'APPROVED' || c.status === 'REJECTED')
    );
    const avgReviewTimeHours = reviewedCerts.length > 0
      ? reviewedCerts.reduce((sum, c) => {
          const reviewTimeMs = c.reviewedAt!.getTime() - c.createdAt.getTime();
  ...
  private async getFieldStats(companyId: string) {
    ...
    const totalHectares = fields.reduce((sum, f) => sum + Number(f.areaHectares), 0);

    const byCropType = fields.reduce((acc, f) => {
  ...
  private async getInspectionStats(companyId: string, range: DateRange) {
    ...
    const photosUploaded = inspections.reduce((sum, i) => sum + i.photos.length, 0);
    const organicInputsTracked = inspections.reduce((sum, i) => sum + i.organicInputs.length, 0);
    ...
    const avgPerFarmer = farmerCount > 0 ? Math.round((thisMonth / farmerCount) * 10) / 10 : 0;
  ...
  private async getTopFarmers(companyId: string, limit: number) {
    ...
    return farmers.map(f => ({
      farmerId: f.id,
      farmerName: f.user ? (f.user.firstName + ' ' + f.user.lastName).trim() : f.businessName || 'Unknown',
      certificatesGenerated: f._count.organicCertificates,
      totalHectares: f.organicFields.reduce((sum: number, field: { areaHectares: any }) => sum + Number(field.areaHectares), 0),
  ...
  private async calculateBillingPreview(companyId: string, range: DateRange) {
    ...
    const overageCount = certificatesIncluded === -1 ? 0 : Math.max(0, certificatesGenerated - certificatesIncluded);
    const overageFees = overageCount * Number(company.certificateFee);
    const projectedTotal = Number(company.monthlyFee) + overageFees;

    return {
      certificatesIncluded: certificatesIncluded === -1 ? 999999 : certificatesIncluded,
   ...
   */
  async getCertificateAnalytics(companyId: string, range: DateRange): Promise<CertificateAnalytics> {
    const certificates = await this.prisma.organicCertificate.findMany({
      where: {
        exportCompanyId: companyId,
        createdAt: { gte: range.start, lte: range.end },
    ...
    // Group by date
    const byDateMap = new Map<string, { count: number; approved: number; rejected: number }>();
    certificates.forEach(c => {
      const dateKey = c.createdAt.toISOString().split('T')[0];
      const existing = byDateMap.get(dateKey) || { count: 0, approved: 0, rejected: 0 };
      existing.count++;
    ...
    // Group by crop type
    const byCropType: Record<string, { count: number; approvalRate: number; avgReviewTimeHours: number }> = {};
    ...
    certificates.forEach(c => {
      byStandard[c.certificationStandard] = (byStandard[c.certificationStandard] || 0) + 1;
    });

    // Rejection reasons
    const rejectedCerts = certificates.filter(c => c.status === 'REJECTED' && c.rejectionReason);
    ...
    const rejectionReasons = Array.from(reasonCounts.entries())
      .map(([reason, count]) => ({
        reason,
        count,
        percentage: rejectedCerts.length > 0 ? Math.round((count / rejectedCerts.length) * 100) : 0,
      }))
    ...
    // Review metrics
    const now = new Date();
    const pendingCerts = certificates.filter(c => c.status === 'PENDING_REVIEW');
    ...
    const approvedCerts = certificates.filter(c => c.status === 'APPROVED' && c.reviewedAt);
    const rejectedCertsWithTime = certificates.filter(c => c.status === 'REJECTED' && c.reviewedAt);

    const avgTimeToApproveHours = approvedCerts.length > 0
      ? Math.round(
          (approvedCerts.reduce((sum, c) =>
            sum + (c.reviewedAt!.getTime() - c.createdAt.getTime()), 0
          ) / approvedCerts.length / (1000 * 60 * 60)) * 10
        ) / 10
      : 0;

    const avgTimeToRejectHours = rejectedCertsWithTime.length > 0
      ? Math.round(
          (rejectedCertsWithTime.reduce((sum, c) =>
            sum + (c.reviewedAt!.getTime() - c.createdAt.getTime()), 0
          ) / rejectedCertsWithTime.length / (1000 * 60 * 60)) * 10
        ) / 10
      : 0;
  ...
  async listCompanyFarmers(
    ...
    const data: FarmerWithStats[] = farmers.map(f => ({
      id: f.id,
      name: f.user ? (f.user.firstName + ' ' + f.user.lastName).trim() : f.businessName,
      email: f.user?.email || '',
      phone: null,
      ...
      pendingCertificates: f.organicCertificates.filter((c: { status: string }) => c.status === 'PENDING_REVIEW').length,
   ...
   */
  async getFarmerDetails(farmerId: string, companyId: string): Promise<FarmerWithStats> {
    ...
    return {
      id: farmer.id,
      name: farmer.user ? (farmer.user.firstName + ' ' + farmer.user.lastName).trim() : farmer.businessName,
      email: farmer.user?.email || '',
      phone: null,
      status: 'ACTIVE',
      enrolledAt: farmer.createdAt,
      fieldsCount: farmer.organicFields.length,
      totalHectares: farmer.organicFields.reduce((sum: number, f: { areaHectares: any }) => sum + Number(f.areaHectares), 0),
      certificatesCount: farmer.organicCertificates.length,
      pendingCertificates: farmer.organicCertificates.filter((c: { status: string }) => c.status === 'PENDING_REVIEW').length,
  ...
  async inviteFarmersBulk(
    ...
    const maxFarmers = company.farmersIncluded === -1 ? 999999 : company.farmersIncluded;

    if (currentFarmerCount + farmersData.length > maxFarmers) {
      throw new FarmerLimitExceededError(maxFarmers, currentFarmerCount, farmersData.length);
    }
    ...
    for (const farmerData of farmersData) {
      try {
        ...
        if (!this.isValidEmail(farmerData.email)) {
          result.failed++;
          result.errors.push({ email: farmerData.email, reason: 'Invalid email format' });
          continue;
        }
        ...
        if (existing) {
          result.failed++;
          result.errors.push({ email: farmerData.email, reason: 'Invitation already pending' });
          continue;
        }
  ...
  async getInvitationStats(companyId: string) {
    const [pending, accepted, expired, cancelled] = await Promise.all([
      ...
      this.prisma.farmerInvitation.count({ where: { exportCompanyId: companyId, status: 'ACCEPTED' } }),
      this.prisma.farmerInvitation.count({ where: { exportCompanyId: companyId, status: 'EXPIRED' } }),
      this.prisma.farmerInvitation.count({ where: { exportCompanyId: companyId, status: 'CANCELLED' } }),
    ]);
  ...
  async getCurrentUsage(companyId: string): Promise<BillingUsage> {
    ...
    const certificatesIncluded = company.certsIncluded === -1 ? 999999 : company.certsIncluded;
   ...
   */
  async generateInvoice(companyId: string, periodStart: Date, periodEnd: Date): Promise<InvoiceSummary> {
    const company = await this.prisma.exportCompany.findUnique({
      where: { id: companyId },
    });
    ...
    if (periodStart >= periodEnd) {
      throw new InvalidBillingPeriodError('Period start must be before period end');
    }
    ...
    // Calculate fees
    const baseFee = Number(company.monthlyFee);
    const certsIncluded = company.certsIncluded === -1 ? 999999 : company.certsIncluded;
