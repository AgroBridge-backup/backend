...
export class TransitTrackingService {
   ...
   */
  async createSession(
    input: CreateTransitSessionInput,
  ): Promise<TransitSession> {
    addBreadcrumb("Creating transit session", "transit", {
    ...
    return instrumentAsync("createSession", "transit.create", async () => {
      ...
      const existingSessions = await this.sessionRepository.findByBatchId(
        input.batchId,
      );
   ...
   */
  async getSessionWithProgress(
    sessionId: string,
  ): Promise<TransitSessionWithProgress | null> {
    const session =
      await this.sessionRepository.findByIdWithLocations(sessionId);

    if (!session) return null;

    const lastLocation =
      await this.sessionRepository.getLatestLocation(sessionId);
    const locationCount =
      await this.sessionRepository.countLocations(sessionId);
  ...
  async addLocationUpdate(
    ...
    addBreadcrumb("Adding location update", "transit", {
    ...
    return instrumentAsync(
      ...
      async () => {
        ...
        const lastLocation = await this.sessionRepository.getLatestLocation(
          input.sessionId,
        );
        ...
        await this.sessionRepository.updateDistanceTraveled(
          input.sessionId,
          distanceTraveledKm,
        );
        ...
        if (input.speed && session.totalDistanceKm) {
          const remainingDistance =
            session.totalDistanceKm - distanceTraveledKm;
          newEstimatedArrival = estimateArrival(remainingDistance, input.speed);

          if (newEstimatedArrival) {
            await this.sessionRepository.updateEstimatedArrival(
              input.sessionId,
              newEstimatedArrival,
            );
          }
        ...
        return {
          location: {
            ...
            deviationKm: location.deviationKm
              ? Number(location.deviationKm)
              : null,
