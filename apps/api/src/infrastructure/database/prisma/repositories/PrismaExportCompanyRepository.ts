/**
 * Prisma ExportCompany Repository Implementation
 * Implements IExportCompanyRepository using Prisma ORM
 */
...
export class PrismaExportCompanyRepository implements IExportCompanyRepository {
  ...
  private mapToDomain(prismaCompany: any): ExportCompany {
    return {
      id: prismaCompany.id,
      ...
      tier: prismaCompany.tier as ExportCompanyTier,
      status: prismaCompany.status as ExportCompanyStatus,
  ...
  async create(data: CreateExportCompanyData): Promise<ExportCompany> {
    const company = await this.prisma.exportCompany.create({
      data: {
        ...
        enabledStandards: data.enabledStandards as CertificationType[],
  ...
  async findByRfc(rfc: string): Promise<ExportCompany | null> {
    const company = await this.prisma.exportCompany.findUnique({
      where: { rfc },
    });
    return company ? this.mapToDomain(company) : null;
  ...
  async findByEmail(email: string): Promise<ExportCompany | null> {
    const company = await this.prisma.exportCompany.findUnique({
      where: { email },
    });
    return company ? this.mapToDomain(company) : null;
  ...
  async list(filter?: ExportCompanyFilter): Promise<ExportCompanyListResult> {
    const where: any = {};
    ...
    if (filter?.tier) {
      where.tier = filter.tier as PrismaTier;
    }
  ...
  async findByIdWithStats(id: string): Promise<ExportCompanyWithStats | null> {
    const company = await this.prisma.exportCompany.findUnique({
      where: { id },
      include: {
        _count: {
  ...
  async countFarmers(id: string): Promise<number> {
  ...
  async findExpiringTrials(daysUntilExpiry: number): Promise<ExportCompany[]> {
  ...
  async cancel(id: string): Promise<ExportCompany> {
    const company = await this.prisma.exportCompany.update({
      where: { id },
      data: {
        status: "CANCELLED",
  ...
  async hasCapacityForFarmers(id: string): Promise<boolean> {
    const company = await this.prisma.exportCompany.findUnique({
      where: { id },
      include: {
        _count: {
    ...
    // Enterprise tier has unlimited farmers
    if (company.farmersIncluded === -1) return true;

    return company._count.farmers < company.farmersIncluded;
  }

  async hasCapacityForCertificates(id: string): Promise<boolean> {
    const company = await this.prisma.exportCompany.findUnique({
      where: { id },
    });
    ...
    // Enterprise tier has unlimited certificates
    if (company.certsIncluded === -1) return true;
