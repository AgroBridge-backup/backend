...
export function getDatabaseConfig(): DatabaseConfig {
  ...
  const connectionUrl = process.env.DATABASE_URL || 'postgresql://localhost:5432/agrobridge';

  // Production: Smaller pool size since pgBouncer handles pooling
  // Development: Larger pool size for parallel test runs
  const poolSize = isProduction || isStaging
...
export const ENV_VARS = {
  DATABASE_URL: 'PostgreSQL connection string (with pgBouncer port in production)',
  DB_POOL_SIZE: 'Number of connections per Prisma client (default: 10 prod, 20 dev)',
