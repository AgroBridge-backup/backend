...
export class PDFGenerator {
  ...
  async generateBatchReport(data: BatchReportData): Promise<Buffer> {
    ...
    this.addKeyValue(
      "Fecha de Cosecha",
      this.formatDate(data.batch.harvestDate),
    );
    this.addKeyValue("Estado", data.batch.status);
    this.addKeyValue(
      "Hash Blockchain",
      data.batch.blockchainHash.substring(0, 20) + "...",
    );
    this.addSpace(20);
  ...
  async generateProducerReport(data: ProducerReportData): Promise<Buffer> {
    ...
    this.addKeyValue(
      "Ubicacion",
      `${data.producer.municipality}, ${data.producer.state}`,
    );
    this.addKeyValue(
      "Estado",
      data.producer.isWhitelisted ? "Verificado" : "Pendiente",
    );
    ...
    this.addStatsGrid([
      ...
      {
        label: "Peso Total",
        value: `${data.stats.totalWeight.toLocaleString()} kg`,
      },
      {
        label: "Certificaciones Activas",
        value: data.stats.activeCertifications.toString(),
      },
      {
        label: "Eventos por Lote",
        value: data.stats.avgEventsPerBatch.toFixed(1),
      },
    ]);
    this.addSpace(20);
    ...
    this.addStatsGrid([
      {
        ...
        value: data.summary.totalActions.toLocaleString(),
      },
  ...
  private initDocument(): void {
    ...
    const layout =
      this.options.orientation === "landscape" ? "landscape" : "portrait";
  ...
  private addHeader(): void {
    ...
    this.doc
      ...
      .text(
        "Plataforma de Trazabilidad Agricola",
        margins.left + 50,
        margins.top - 28,
      );
  ...
  private addTable(
    ...
    const defaultWidth =
      undefinedCount > 0 ? remainingWidth / undefinedCount : 0;
    ...
    this.doc.font(this.fonts.bold).fontSize(9).fillColor(this.colors.text);
    ...
    data.forEach((row, rowIndex) => {
      ...
      if (rowIndex % 2 === 1) {
        this.doc!.rect(margins.left, rowY - 3, tableWidth, 20).fill("#FAFAFA");
      }
  ...
  private addStatsGrid(stats: Array<{ label: string; value: string }>): void {
    ...
    const margins = this.options.margins!;
    const boxWidth =
      (this.doc.page.width - margins.left - margins.right - 30) / stats.length;
    const boxHeight = 60;
    ...
    stats.forEach((stat, i) => {
      ...
      // Box background
      this.doc!.roundedRect(x, this.yPosition, boxWidth, boxHeight, 5).fill(
      ...
      this.doc!.font(this.fonts.bold)
        .fontSize(20)
        .fillColor(this.colors.primary)
        .text(stat.value, x, this.yPosition + 10, {
          width: boxWidth,
          align: "center",
        });
      ...
      this.doc!.font(this.fonts.regular)
        .fontSize(9)
        .fillColor(this.colors.textLight)
        .text(stat.label, x, this.yPosition + 38, {
          width: boxWidth,
          align: "center",
        });
    });
  ...
  private addPageNumber(): void {
    ...
    this.doc
      ...
      .text(
        ...
        {
          align: "center",
          width: this.doc.page.width - margins.left - margins.right,
        },
      );
  }
  ...
  private checkPageBreak(requiredSpace: number): void {
    ...
    const availableSpace =
      this.doc.page.height - margins.bottom - this.yPosition;
