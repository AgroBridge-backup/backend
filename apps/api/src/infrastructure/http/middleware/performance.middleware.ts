...
const CRITICAL_REQUEST_THRESHOLD = parseInt(
  process.env.CRITICAL_REQUEST_THRESHOLD || "5000",
  10,
);
...
export const performanceMiddleware = (
  ...
  res.end = function (
    ...
    if (durationMs >= CRITICAL_REQUEST_THRESHOLD) {
      logger.error(
        `[Performance] CRITICAL: Very slow request - ${req.method} ${req.path} took ${durationMs}ms`,
      );
    } else if (durationMs >= SLOW_REQUEST_THRESHOLD) {
      logger.warn(
        `[Performance] Slow request - ${req.method} ${req.path} took ${durationMs}ms`,
      );
    } else if (res.statusCode >= 500) {
      logger.error(
        `[Performance] Server error - ${req.method} ${req.path} returned ${res.statusCode} in ${durationMs}ms`,
      );
    } else if (res.statusCode >= 400) {
      logger.warn(
        `[Performance] Client error - ${req.method} ${req.path} returned ${res.statusCode} in ${durationMs}ms`,
      );
    } else {
      logger.debug(
        `[Performance] ${req.method} ${req.path} completed in ${durationMs}ms`,
      );
    }
...
export const requestSizeMiddleware = (
  ...
  if (contentLength > 1024 * 1024) {
    // > 1MB
    logger.warn(
      `[Performance] Large request body: ${contentLength} bytes for ${req.method} ${req.path}`,
    );
  }
