/**
 * @file Analytics GraphQL Resolvers
 * @description Resolvers for analytics and dashboard queries
 *
...
export const analyticsQueries = {
  dashboard: async (_parent: unknown, _args: unknown, context: GraphQLContext) => {
    ...
    // Calculate percentages for event type distribution
  ...
  topProducers: async (
    ...
    const results = await Promise.all(
      topProducers.map(async (item, index) => {
  ...
  eventDistribution: async (
    ...
    if (args.dateRange) {
      where.timestamp = {};
      if (args.dateRange.from) where.timestamp.gte = args.dateRange.from;
      if (args.dateRange.to) where.timestamp.lte = args.dateRange.to;
    }
