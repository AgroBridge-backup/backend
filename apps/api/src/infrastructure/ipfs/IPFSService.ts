...
export class IpfsService {
  ...
  async uploadCertificateMetadata(
    ...
    if (!this.isConfigured()) {
      logger.warn("IPFS service not configured, skipping upload", {
        certificateNumber,
      });
      return null;
    }
  ...
  async isPinned(ipfsHash: string): Promise<boolean> {
    ...
    try {
      const response = await fetch(
        `${this.pinataApiUrl}/pinning/pinJobs?ipfs_pin_hash=${ipfsHash}`,
        {
          method: "GET",
          headers: {
            pinata_api_key: this.apiKey,
      ...
      logger.warn("Failed to verify IPFS pin status", {
        ipfsHash,
        error: error.message,
      });
      return false;
...
export function createIpfsService(): IpfsService {
  return new IpfsService({
    ...
    gateway:
      (process.env.IPFS_GATEWAY as keyof typeof IPFS_GATEWAYS) || "pinata",
  });
}
