...
export class PdfGenerator {
  ...
  constructor(config?: Partial<PdfGeneratorConfig>) {
    this.config = {
      ...
      verificationBaseUrl: config?.verificationBaseUrl || 'https://verify.agrobridge.io',
    };
  }
  ...
  private renderPage1(doc: PDFKit.PDFDocument, data: PdfTemplateData): void {
    ...
    // Certificate title
    doc.fontSize(16)
      .text('CERTIFICADO DE PRODUCCION ORGANICA', 50, 70);
    ...
    const standardName = CERTIFICATION_DISPLAY_NAMES[data.certificationStandard as string] ||
      data.certificationStandard;

    doc.rect(50, y, 200, 30)
      .fillAndStroke('#2d7a3e', '#1a5f2a');
    ...
    const infoRows = [
      ...
      ['Superficie / Area:', `${data.farmer.totalHectares.toFixed(2)} hectareas`],
      ['Cultivo / Crop:', data.cropType],
    ];

    if (data.harvestDate) {
      infoRows.push(['Fecha Cosecha / Harvest:', format(data.harvestDate, 'dd MMMM yyyy', { locale: es })]);
    }
    ...
    doc.font('Helvetica-Bold')
      .fontSize(24)
      .fillColor('#1a5f2a')
      .text(data.inspectionSummary.totalInspections.toString(), 50, summaryY, { width: colWidth, align: 'center' });

    doc.fontSize(10)
      .fillColor('#666666')
      .text('Inspecciones', 50, summaryY + 30, { width: colWidth, align: 'center' })
      .text('Inspections', 50, summaryY + 42, { width: colWidth, align: 'center' });

    // Column 2: Photos
    doc.font('Helvetica-Bold')
      .fontSize(24)
      .fillColor('#1a5f2a')
      .text(data.inspectionSummary.photosCount.toString(), 50 + colWidth, summaryY, { width: colWidth, align: 'center' });

    doc.fontSize(10)
      .fillColor('#666666')
      .text('Fotografias', 50 + colWidth, summaryY + 30, { width: colWidth, align: 'center' })
      .text('Photos', 50 + colWidth, summaryY + 42, { width: colWidth, align: 'center' });

    // Column 3: Organic Inputs
    doc.font('Helvetica-Bold')
      .fontSize(24)
      .fillColor('#1a5f2a')
      .text(data.inspectionSummary.organicInputsCount.toString(), 50 + colWidth * 2, summaryY, { width: colWidth, align: 'center' });

    doc.fontSize(10)
      .fillColor('#666666')
      .text('Insumos Organicos', 50 + colWidth * 2, summaryY + 30, { width: colWidth, align: 'center' })
      .text('Organic Inputs', 50 + colWidth * 2, summaryY + 42, { width: colWidth, align: 'center' });
    ...
    // Verification badge
    doc.rect(pageWidth - 80, blockchainY + 10, 80, 40)
      .fillAndStroke('#1a5f2a', '#0f3a18');

    doc.fillColor('#FFFFFF')
      .fontSize(10)
      .font('Helvetica-Bold')
      .text('VERIFICADO', pageWidth - 80, blockchainY + 20, { width: 80, align: 'center' })
      .fontSize(8)
      .font('Helvetica')
      .text('VERIFIED', pageWidth - 80, blockchainY + 34, { width: 80, align: 'center' });
  ...
  private renderPage2(doc: PDFKit.PDFDocument, data: PdfTemplateData): void {
    ...
    if (data.fieldMapImageUrl) {
      ...
      y += 25;
      doc.rect(50, y, pageWidth, 150)
        .stroke('#cccccc');

      doc.fillColor('#999999')
        .fontSize(10)
        .text('[Mapa de Ubicacion / Location Map]', 50, y + 65, { width: pageWidth, align: 'center' });
    ...
    doc.fillColor('#1a5f2a')
      .fontSize(10)
      .font('Helvetica-Bold')
      .text('Verificar certificado en / Verify certificate at:', 50, footerY + 15);

    doc.fillColor('#2d7a3e')
      .fontSize(11)
      .font('Helvetica')
      .text(`${this.config.verificationBaseUrl}/${data.certificateNumber}`, 50, footerY + 30);
 ...
 */
export function createPdfGenerator(config?: Partial<PdfGeneratorConfig>): PdfGenerator {
