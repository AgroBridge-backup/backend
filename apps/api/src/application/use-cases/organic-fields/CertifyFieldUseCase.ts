...
export class CertifyFieldUseCase {
  ...
  async execute(request: CertifyFieldRequest): Promise<CertifyFieldResponse> {
    ...
    if (!request.standards || request.standards.length === 0) {
      throw new ValidationError('At least one certification standard is required');
    }
    ...
    for (const standard of request.standards) {
      if (!validStandards.includes(standard)) {
        throw new ValidationError(`Invalid standard: ${standard}. Valid: ${validStandards.join(', ')}`);
      }
    }
    ...
    const field = await this.fieldService.certifyField(request.fieldId, request.standards);
