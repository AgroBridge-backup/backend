...
} from "./infrastructure/http/middleware/security.middleware.js";
...
} from "./infrastructure/http/middleware/error-tracking.middleware.js";
...
export function createApp(injectedRouter?: Router): express.Express {
  ...
  if (!apiRouter) {
    ...
    // Traceability 2.0 - Verification Stages
    const verificationStageRepository = new PrismaVerificationStageRepository(
      prisma,
    );
    const verificationStageService = new VerificationStageService(
      verificationStageRepository,
    );
    const stageFinalizationService = new StageFinalizationService(
      prisma,
      verificationStageService,
    );

    // Traceability 2.0 - Quality Certificates
    const certificateRepository = new PrismaQualityCertificateRepository(
      prisma,
    );
    ...
    const transitTrackingService = new TransitTrackingService(
      prisma,
      transitSessionRepository,
    );
    ...
    const useCases: AllUseCases = {
      auth: {
        ...
        verify2FAUseCase: new Verify2FAUseCase(
          userRepository,
          refreshTokenRepository,
        ),
        get2FAStatusUseCase: new Get2FAStatusUseCase(userRepository),
        regenerateBackupCodesUseCase: new RegenerateBackupCodesUseCase(
          userRepository,
        ),
      },
      producers: {
      ...
      // Traceability 2.0 - Verification Stages
      verificationStages: {
        getBatchStagesUseCase: new GetBatchStagesUseCase(
          verificationStageService,
        ),
        createBatchStageUseCase: new CreateBatchStageUseCase(
          verificationStageService,
        ),
        updateBatchStageUseCase: new UpdateBatchStageUseCase(
          verificationStageService,
        ),
        finalizeBatchStagesUseCase: new FinalizeBatchStagesUseCase(
          stageFinalizationService,
        ),
      },
      // Traceability 2.0 - Quality Certificates
      certificates: {
        issueCertificateUseCase: new IssueCertificateUseCase(
          certificateService,
        ),
        getCertificateUseCase: new GetCertificateUseCase(certificateService),
        listBatchCertificatesUseCase: new ListBatchCertificatesUseCase(
          certificateService,
        ),
        verifyCertificateUseCase: new VerifyCertificateUseCase(
          certificateService,
        ),
        checkCertificateEligibilityUseCase:
          new CheckCertificateEligibilityUseCase(certificateService),
      },
      // Traceability 2.0 - Real-Time Transit Tracking
      transit: {
        createTransitSessionUseCase: new CreateTransitSessionUseCase(
          transitTrackingService,
        ),
        getTransitSessionUseCase: new GetTransitSessionUseCase(
          transitTrackingService,
        ),
        updateTransitStatusUseCase: new UpdateTransitStatusUseCase(
          transitTrackingService,
        ),
        addLocationUpdateUseCase: new AddLocationUpdateUseCase(
          transitTrackingService,
        ),
        getLocationHistoryUseCase: new GetLocationHistoryUseCase(
          transitTrackingService,
        ),
