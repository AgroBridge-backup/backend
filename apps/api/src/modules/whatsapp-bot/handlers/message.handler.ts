...
export class MessageHandler {
    ...
    switch (intent) {
      ...
      case "CUSTOMER_SUPPORT":
        await whatsAppService.sendText(
          phone,
          getMessage("supportMessage", lang),
        );
  ...
  private async sendWelcome(
    ...
    await whatsAppService.sendText(
      phone,
      getMessage("welcome", lang, userName),
    );
    await this.sendMainMenu(phone, lang);
   ...
   */
  private async handleAdvanceRequest(
    phone: string,
    userName: string,
  ): Promise<void> {
    const session = await sessionManager.getSession(phone);
    ...
    if (eligibleOrders.length === 0) {
      await whatsAppService.sendText(
        phone,
        getMessage("noEligibleOrders", lang),
      );
      await this.sendMainMenu(phone, lang);
      return;
    }
    ...
    await whatsAppService.sendListMenu(
      ...
      [
        {
          title: lang === "es" ? "Órdenes Elegibles" : "Eligible Orders",
          items: orderOptions,
        },
      ],
    );
   ...
   */
  private async handleOrderSelection(
    phone: string,
    orderId: string,
  ): Promise<void> {
    const session = await sessionManager.getSession(phone);
    ...
    await whatsAppService.sendText(
      phone,
      getMessage("advanceCalculation", lang, data),
    );
    ...
    await whatsAppService.sendButtons(
      ...
      [
        {
          id: "confirm_yes",
          title: lang === "es" ? "✅ Sí, confirmar" : "✅ Yes, confirm",
        },
        {
          id: "confirm_no",
          title: lang === "es" ? "❌ No, cancelar" : "❌ No, cancel",
        },
      ],
    );
  ...
  private async handleConfirmation(
    ...
    if (
      session.state !== "CONFIRMING_ADVANCE" ||
      !session.context.selectedOrderId
    ) {
      await this.sendMainMenu(phone, lang);
      return;
    }
    ...
    try {
      ...
      await whatsAppService.sendText(
        ...
        getMessage(
          ...
          lang,
          contractNumber,
          `$${advanceAmount.toLocaleString()} MXN`,
        ),
      );
  ...
  private async handleBalanceCheck(phone: string): Promise<void> {
    ...
    if (advances.length === 0) {
      await whatsAppService.sendText(
        phone,
        getMessage("noActiveAdvances", lang),
      );
      await this.sendMainMenu(phone, lang);
      return;
    }

    const totalPending = advances.reduce(
      (sum, a) => sum + a.remainingBalance.toNumber(),
      0,
    );
    ...
    const data = {
      ...
      nextPaymentDate: nextAdvance.dueDate.toLocaleDateString(
        lang === "es" ? "es-MX" : "en-US",
      ),
    ...
    await whatsAppService.sendText(
      phone,
      getMessage("balanceSummary", lang, data),
    );
    await this.sendMainMenu(phone, lang);
  }
  ...
  private async handlePaymentMethod(
    ...
    await whatsAppService.sendText(
      phone,
      getMessage("paymentLink", lang, amount, paymentLink),
    );
  ...
  private async sendNotUnderstood(phone: string): Promise<void> {
    ...
    await whatsAppService.sendText(
      phone,
      getMessage("notUnderstood", session.language),
    );
  }
  ...
  private async sendError(phone: string): Promise<void> {
    ...
    await whatsAppService.sendText(
      phone,
      getMessage("genericError", session.language),
    );
  }
}
