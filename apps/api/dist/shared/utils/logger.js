...
const formats = winston.format.combine(winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), winston.format.errors({ stack: true }), // Log the full stack trace
winston.format.splat(), isProduction
    ? winston.format.json() // Structured JSON for production
    : winston.format.combine(
    // Human-readable for development
    winston.format.colorize(), winston.format.printf(({ level, message, timestamp, stack, traceId, userId, ...meta }) => {
        let msg = message;
 ...
 * Useful for maintaining trace context throughout request lifecycle
 */
export const createChildLogger = (traceId) => {
