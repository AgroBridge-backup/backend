...
router.get('/leaderboard/:month/:year', async (req, res) => {
    try {
        ...
        if (entries.length === 0) {
            ...
            entries = await Promise.all(codes.map(async (code, index) => {
...
function generateBatchVerificationHTML(data) {
...
function generateCertificateVerificationHTML(data) {
...
function generateEventVerificationHTML(data) {
...
function generateReferralVerificationHTML(data) {
...
function generateInvoiceVerificationHTML(data) {
