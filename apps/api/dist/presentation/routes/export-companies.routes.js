...
import { RegisterExportCompanyUseCase, GetExportCompanyUseCase, ListExportCompaniesUseCase, UpdateExportCompanyUseCase, UpgradeTierUseCase, CheckCapacityUseCase, } from '../../application/use-cases/export-companies/index.js';
...
export function createExportCompaniesRouter(prisma) {
    const router = Router();
    ...
    router.post('/', authenticate(['ADMIN']), RateLimiterConfig.creation(), async (req, res, next) => {
        try {
    ...
    router.get('/', authenticate(['ADMIN']), RateLimiterConfig.api(), async (req, res, next) => {
        try {
    ...
    router.get('/:id', authenticate(['ADMIN']), RateLimiterConfig.api(), async (req, res, next) => {
        try {
            const { id } = req.params;
    ...
    router.patch('/:id', authenticate(['ADMIN']), RateLimiterConfig.api(), async (req, res, next) => {
        try {
            const { id } = req.params;
    ...
    router.post('/:id/upgrade', authenticate(['ADMIN']), RateLimiterConfig.api(), async (req, res, next) => {
        try {
            const { id } = req.params;
    ...
    router.get('/:id/capacity', authenticate(['ADMIN']), RateLimiterConfig.api(), async (req, res, next) => {
        try {
            const { id } = req.params;
    ...
    router.get('/tiers/config', authenticate(['ADMIN']), RateLimiterConfig.api(), async (req, res, next) => {
        try {
            res.json({
                success: true,
                data: {
