...
export class SimpleCreditScoringService {
    ...
    try {
      await prisma.creditScore.upsert({
        ...
        create: {
          ...
          // Store factors as JSON
          deliveryMetrics: result.factors.transactionFrequency as any,
          qualityMetrics: result.factors.profileCompleteness as any,
          paymentMetrics: result.factors.repaymentHistory as any,
          activityMetrics: result.factors.requestPattern as any,
          blockchainMetrics: result.factors.externalSignals as any,
        },
        update: {
          overallScore: result.score,
          riskTier: result.tier === 'A' ? 'A' : result.tier === 'B' ? 'B' : 'C',
      ...
      await prisma.creditScoreHistory.create({
        data: {
          creditScoreId: result.producerId, // This is a simplification
  ...
  async getStoredScore(producerId: string): Promise<CreditScoreResult | null> {
    ...
    return {
      userId: '',
      producerId,
      score: score.overallScore.toNumber(),
      tier: score.riskTier as 'A' | 'B' | 'C',
      decision: this.determineDecision(score.overallScore.toNumber()).decision,
      maxApprovedAmount: this.determineDecision(score.overallScore.toNumber()).maxAmount,
      factors: {
        repaymentHistory: score.paymentMetrics as any,
        transactionFrequency: score.deliveryMetrics as any,
        profileCompleteness: score.qualityMetrics as any,
        requestPattern: score.activityMetrics as any,
        externalSignals: score.blockchainMetrics as any,
      },
      breakdown: {
        repaymentScore: 0,
        transactionScore: 0,
        ...
        patternScore: 0,
        externalScore: 0,
        totalScore: score.overallScore.toNumber(),
      },
