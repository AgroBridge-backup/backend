...
export async function runSeed() {
  ...
  logger.info(`✅ Liquidity Pool created: ${pool.name} ($${pool.totalCapital})`);
  ...
  const createdProducers: Array<{ id: string; email: string; producerId: string }> = [];
  ...
  for (const farmerData of farmersData) {
    ...
    logger.info(`✅ Farmer created: ${farmerData.firstName} ${farmerData.lastName} (Score: ${farmerData.creditScore.overallScore}, Tier ${farmerData.creditScore.riskTier})`);
  }
  ...
  for (let i = 0; i < ordersToCreate.length; i++) {
    ...
    await prisma.order.upsert({
      ...
      create: {
        ...
        currency: 'MXN',
        status: 'CONFIRMED',
        expectedDeliveryDate: new Date(Date.now() + orderData.deliveryDays * 24 * 60 * 60 * 1000),
        advanceEligible: true,
        advanceRequested: false,
      },
    });

    logger.info(`✅ Order created: ${orderNumber} - $${totalAmount} MXN (${orderData.productType})`);
  }

  // --- Summary ---
  logger.info('\n📊 Cash Flow Bridge MVP Seed Summary:');
  logger.info(`   💰 Liquidity Pool: $${pool.totalCapital} available`);
  logger.info(`   👨‍🌾 Farmers: ${createdProducers.length} (1 Tier A, 3 Tier B, 1 Tier C)`);
  logger.info(`   📦 Orders: ${ordersToCreate.length} confirmed and advance-eligible`);
