...
export function createBatchesRouter(useCases: BatchUseCases) {
  ...
  router.post('/', authenticate([Prisma.UserRole.PRODUCER]), validateRequest(createBatchSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    console.log("[DEBUG] Batch POST start");
    try {
      ...
      if (!result || !result.id) {
        console.error("[DEBUG] Batch POST: Use case did not return a valid batch with an ID. Returning 400.");
        return res.status(400).json({ error: "Batch creation failed, use case returned invalid data." });
      }

      console.log("[DEBUG] Batch POST: created batch with ID", result.id);
      res.status(201).json(result);
    } catch (error) {
      console.error("[DEBUG] Batch POST error:", error);
      next(error);
    }
  });

  router.get('/:id', authenticate(), async (req: Request, res: Response, next: NextFunction) => {
    console.log("[DEBUG] Batch GET by ID start for ID:", req.params.id);
    try {
      const result = await useCases.getBatchByIdUseCase.execute({ id: req.params.id });

      if (!result) {
        console.log("[DEBUG] Batch GET: Batch not found, returning 404.");
        return res.status(404).json({ error: "Batch not found" });
      }

      console.log("[DEBUG] Batch GET: Found batch:", result.id);
      res.status(200).json(result);
    } catch (error) {
      console.error("[DEBUG] Batch GET error:", error);
      next(error);
    }
  });
