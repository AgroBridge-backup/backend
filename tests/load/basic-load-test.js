...
export function setup() {
  ...
  console.log(`Server version: ${healthData.version}, environment: ${healthData.environment}`);
...
export default function (data) {
  ...
  group('API Endpoints', () => {
    ...
    const infoRes = http.get(`${data.baseUrl}/api/v1`, {
    ...
    check(authRes, {
      'Auth endpoint responds (401 expected)': (r) => r.status === 401 || r.status === 403 || r.status === 404,
    });
  });
...
function textSummary(data) {
  ...
  if (metrics.health_check_duration) {
    ...
    summary += `  - avg: ${metrics.health_check_duration.values.avg.toFixed(2)}ms\n`;
    summary += `  - p99: ${metrics.health_check_duration.values['p(99)'].toFixed(2)}ms\n\n`;
  }
  ...
  if (data.root_group && data.root_group.checks) {
    ...
    checks.forEach(c => {
      const status = c.fails === 0 ? '✅' : '❌';
      summary += `  ${status} ${c.name}: ${c.passes}/${c.passes + c.fails} passed\n`;
    });
  }
